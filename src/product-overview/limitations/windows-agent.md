...
### Disk Volume Requirements
   ...
   - Disks that do not need to be synchronized (except for the boot disk) can be excluded through configuration, and these exclusions only take effect for entire disks as the system does not currently support specific partition exclusion. Reference Document：[Source Disk Sync Settings](../../userguide/dr/dr/host-dr.md#source-disk-sync-settings)。**Note: These modifications must be performed before synchronization begins. Once the data synchronization task has started, the settings cannot be changed.**
   ![](./images/windows-agent-1.png)
