...
Resource Migration is the main workflow guide page for HyperMotion Migration. All Migration operations are completed on this page.

The host Migration process consists of three main steps: Select Host, Setup Migration, and Start Migration. You can follow these three steps to complete the full cycle from host Migration to Migration drill/takeover.

Click the top "Migration" navigation bar, then click the left "Host Migration" navigation bar to perform the main host Migration workflow.

## **Operation Process**

| **Step**           | **Description**                                                                                      |
| ------------------ | --------------------------------------------------------------------------------------------------- |
| Preparation        | Register HyperMotion user and add HyperMotion product license                                             |
| Step 1 Select Host | Click the top **"Migration"** navigation bar, click the left **"Host Migration"**, then click **"Select Host"** to start the host selection step |
| Step 2 Setup Migration    | Click the top **"Migration"** navigation bar, click the left **"Host Migration"**, then click **"Setup Migration"** to start the Migration configuration step |
| Step 3 Start Migration    | Click the top **"Migration"** navigation bar, click the left **"Host Migration"**, then click **"Start Migration"** to start the Migration process step         |

## **Preparation**

### **Register HyperMotion User**

Before adding Migration hosts, you need to register or add a Migration account. You can use the default admin account created during installation to log in to the platform, or create a new Migration account. The platform supports multi-tenant mode.

* **Operations Management Platform Address**: `https://<IP>:30443`
![](./images/login-loginportal-2.png)

(Create a new Migration account)

If you already have an account, please skip this step.

### **Add HyperMotion Product License**

After the Migration product is installed, you need to add a Migration product license. You can contact your project manager or send an email to <support@oneprocloud.com> to apply for a valid license.
...
This step applies when the disaster recovery (Migration) target platform has already been automated and integrated. You need to complete the **Migration target configuration** in advance.

If you have not configured it yet, please first go to **(Disaster Recovery Target Platform Configuration)** to add a Migration recovery platform.

If you are using object storage mode, you can skip this step.

### **Network Policy Activation**

When performing **Select Host, Migration Configuration, and Start Migration** steps, you must activate network access from the source production platform to the HyperMotion Migration console and the Migration target. This ensures the source production platform can be properly added, registered, and can synchronize data. Without this, adding hosts will fail.

* (Network architecture and policies for agentless mode)

* (Network architecture and policies for agent-based mode)

### **Plan VPC, Subnets, and Other Network Resources for the Migration Target Platform**

During **Migration configuration**, you need to assign target resources for the backup hosts. This requires selecting pre-planned VPCs, subnets, security groups, and other network settings. If these resources have not been created and planned ahead, subsequent steps cannot proceed.

You must ensure there is at least one usable VPC, subnet, and security group under the Migration target cloud account.

This step is applicable for Migration targets using automated block storage mode or object storage automated recovery mode. Other common methods can ignore this step.

## **Select Host**

Log in to the console, click the top **"Migration"** navigation bar, then click **"Host Migration"** on the left sidebar. Next, click the **"Select Host"** menu, and click the **"Add Host"** button to add production hosts for disaster recovery.
...
Log in to the console, click the top **"Migration"** navigation bar, then click **"Host Migration"** on the left. Click the **"Setup Migration"** menu, select one or multiple hosts, and click the **"Setup Migration"** button to configure disaster recovery.
...
| ----------------------- | ------------------ | ------------------------------------------------------------------------------------------------ |
| Select Storage Type     | **Block Storage**  | Prerequisite: Block storage must be pre-configured; select the configured block storage for Migration backup |
...
After completing the block storage setup, you can select the corresponding block storage platform from the dropdown list,After completing **Specify Migration Platform**, click **"Next"** to start **Specify Cloud Sync Gateway**.

#### **Specify Cloud Sync Gateway**

Select the virtual machines that require disaster recovery backup from the current Migration platform, assign the corresponding disaster recovery host disks, and complete the configuration of the sync gateway for the respective cloud platform.

> Note: The cloud sync gateway will be automatically created when adding the Migration platform configuration, no manual configuration is needed. For details, refer to: (Storage Configuration -- Block Storage Configuration -- Find your corresponding cloud vendor configuration method (not available yet))

![](./images/hostdisasterrecovery-hostdisasterrecovery-29.png)

Click the **"+"** next to the corresponding disaster recovery host to assign the Migration host.
...
The system supports both IPv4 and IPv6 networks. Please select the corresponding network and subnet types according to the deployment requirements of the Migration hosts.
...
| Parameter | Configuration            | Description                                                      |
| --------- | ------------------------ | ---------------------------------------------------------------- |
| Network   | Target Migration network name   | Specify the target network connected to the Migration host for recovery communication. |
| Subnet    | Target associated subnet | Specify the subnet within the target network through which the Migration host connects. |
...
After **Associating Policy**, click **"Confirm"**, Before configuring the policy, the pre-configured hosts will automatically enter the "Start Migration" phase.
...
After completing **Policy Association**, click **"Confirm"**. The backup host will proceed to Step 3 to continue with the **"Start Migration"** operation.

## **Start Migration**

Log in to the console, click the top navigation **"Disaster Recovery"**, then click **"Host Migration"** on the left panel. Click **"Start Migration"**, select one or multiple hosts, and click **"Sync Now"** to initiate the disaster recovery process.
...
Select the disaster recovery host, click “Drill”, choose a recovery point, and restore the Migration host to the target environment with one click.
...
Click **"Specify Cloud Sync Gateway"** to modify the Migration platform. For details, refer to:
...
Click **"Specify Volume Type"** to modify the Migration platform. For details, refer to:
...
Click **"Computing Resource Configuration"** to modify the Migration platform. For details, refer to:
...
Click **"Network Configuration"** to modify the Migration platform. For details, refer to:
...
Click **"Advanced Configuration"** to modify the Migration platform. For details, refer to:
...
Click **"Associate Policy"** to modify the Migration platform. For details, refer to:
