...
| Image                         | ubuntu-24.04-XXXXXXXXXXXXX                               | When creating a cloud sync gateway, select a Linux OS image. If Ubuntu 24.04 is not available in the dropdown, you can manually upload: 1. Upload the image file to object storage; 2. Import from object storage as a custom image. **Note:** To avoid irregular image naming, all image information is listed here. Please make sure to select the required OS image. |
| Flavor                        | 4C\_8GB 50GB(4C8G)                                       | The cloud sync gateway is mainly used for data synchronization. It is recommended to select an instance with at least 4 vCPU and 8 GB memory, and ensure the bandwidth is above 1Gbps for efficient and stable data synchronization. |
