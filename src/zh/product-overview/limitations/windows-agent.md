...
### 磁盘卷要求
   ...
   - 不需要同步的磁盘（除启动磁盘外）可通过配置进行排除。且排除配置仅针对完整磁盘生效，暂不支持排除磁盘内的单个分区。参考文档：[源端磁盘同步设置](../../userguide/dr/dr/host-dr.md#源端磁盘同步设置)。**注意：相关修改操作必须在同步前进行。数据同步任务开始后，设置将不可更改。**
...
| C:\\ProgramData\\CloudSyncAgent | 目录 |
