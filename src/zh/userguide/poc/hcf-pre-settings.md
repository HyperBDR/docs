# Huawei Cloud Foundation Pre-Settings

[[toc]]

## Preparing a Huawei Cloud Foundation Account

Prepare a Huawei Cloud Foundation platform account. The account must have the control permissions to use the compute, storage, network, and image functions of the platform.
...
## Obtain Huawei Cloud HCF 1.0.x Platform Credentials information

When targeting the Huawei Cloud HCF 1.0.x version, the following authentication information of the HCF platform is required to complete the authentication docking of the API interface.  

[How to obtain Huawei Cloud HCF 1.0.x Platform Credentials information?](https://docs.oneprocloud.com/userguide/faq/faq.html#how-to-obtain-huawei-cloud-hcf-1-0-x-platform-credentials-information)

## Configure VPC & Subnet

Establish your network based on the following disaster recovery network scenarios.  
[Network Planning Overview](https://docs.oneprocloud.com/product-overview/presales/dr-network-planning-recommendations.html#deployment-solution-for-block-storage)
...
::: tip
We need to use three image files to import four images in Huawei Cloud Foundation.
...
Contact Huawei Support to import the required image to the Huawei Cloud Foundation platform as a usable image.
...
- Step1: Login to Huawei Cloud Foundation and create an ECS test cloud server  
Create an ECS test cloud server (CentOS 7.6 operating system) on Huawei Cloud Foundation. When configuring the network, choose the business VPC and subnet. Ensure that the security group of this newly created ECS test virtual machine has an inbound access policy allowing port 22.

- Step2: Network testing access from the disaster recovery VPC to the business VPC

Log in to the Huawei Cloud Foundation console, access the HyperBDR ECS cloud server, and use the intranet IP to connect to the ECS host within the business VPC.
...
### Huawei Cloud Foundation account resource quota check

**View resource quotas**
Compile your Huawei Cloud Foundation account resource quotas and calculate the remaining quota
...
### Increase quotas in your Huawei Cloud Foundation account.

If it is discovered that the remaining resource quota in the Huawei Cloud Foundation account is insufficient, and there are no resources to clean up in the account, you can try submitting a Huawei Cloud Foundation ticket to request an expansion of resource quotas.
...
Log in to Huawei Cloud Foundation and use Link Address to find the cloud sync gateway instance.
